<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>

    <link href="https://fonts.googleapis.com/css2?family=Quicksand:wght@400;700&display=swap"
          rel="stylesheet">
    <link rel="stylesheet" href="../styles/global.css">
    <link rel="stylesheet" href="../styles/pages/pages.css">

    <style>
        * {
            color: white;
        }
    </style>
</head>

<body>
<h1 id="header">About Me</h1>
<div class="wrapper aboutme">
    <h3>Who Am I</h3>
    <p>I am currently a second year student at NIPTICT and also a member of Erobot Team.</p>
    <h3>Profile</h3>
    <table>
        <tr>
            <td>Name:</td>
            <td>Choem Thea</td>
        </tr>
        <tr>
            <td>Birth of Date:</td>
            <td>8 December 2001</td>
        </tr>
        <tr>
            <td>Gender:</td>
            <td>Male</td>
        </tr>
    </table>
    <h3>Educations</h3>
    <hr>
    <ul id="timeline">
        <li>2019 - Present
            <h3><a href="https://niptict.edu.kh/" style="cursor:pointer;" target="_blank">National Institute of Post,
                    Telecom, and ICT</a></h3>
            <p>Studies Computer Science since Oct 28th, 2019</p>
        </li>
        <li>2013
            <h3><a href="https://facebook.com/sompovpounhighschool/" style="cursor:pointer;" target="_blank">Hun Sen
                    Sompoeupoun High School</a></h3>
            <p>Studied General Knowledge</p>
        </li>
        <li>2007
            <h3>Koh Teav Primary School</h3>
            <p>Started survive until 2012</p>
        </li>
    </ul>
</div>
<a class="view-cv" href="../assets/pdf/curriculum-vitae.pdf" target="_blank">View CV</a>
</body>

</html>